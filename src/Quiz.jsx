...
const Quiz = () => {
  ...
  const [Qdata, setQdata] = useState(null); // State to hold the quiz data

  // Fetch the quiz data from localStorage if available
  useEffect(() => {
    const storedQuestions = JSON.parse(localStorage.getItem(`questions_${topic}`));

    if (storedQuestions) {
      setQdata({ [topic]: storedQuestions });
    } else {
      // If no questions in localStorage, fallback to fetching the Qdata.json file
      fetch('/Qdata.json') // Path from the root of the public folder
        .then((response) => response.json())
        .then((data) => setQdata(data)) // Set the data when it is fetched
